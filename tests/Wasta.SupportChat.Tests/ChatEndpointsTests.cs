...
public class ChatEndpointsTests : IDisposable
{
    ...
    public async Task CreateSession_ReturnsNewSessionId()
    {
        ...
        var result = await ChatEndpoints.CreateSessionAsync(
            new CreateSessionRequest { VisitorId = "visitor-1" },
            AnyUser(),
            new FakeCurrentStudentAccessor(null),
            service,
    ...
    public async Task SendMessage_UnknownSession_Returns404()
    {
        ...
        var result = await ChatEndpoints.SendMessageAsync(
            Guid.NewGuid(), new SendMessageRequest { Message = "hi" }, service, CancellationToken.None);
    ...
    public async Task SendMessage_KnownSession_ReturnsAnsweredOutcomeAndReply()
    {
        ...
        var result = await ChatEndpoints.SendMessageAsync(
            session.PublicId, new SendMessageRequest { Message = "How does scoring work?" }, service, CancellationToken.None);
    ...
    public async Task GetHistory_ReturnsMessagesMappedToLowercaseRoles()
    {
        ...
        await service.SendMessageAsync(session.PublicId, "A question", CancellationToken.None);

        var result = await ChatEndpoints.GetHistoryAsync(session.PublicId, service, CancellationToken.None);
    ...
    public async Task GetHistory_UnknownSession_ReturnsEmptyListNotError()
    {
        ...
        var result = await ChatEndpoints.GetHistoryAsync(Guid.NewGuid(), service, CancellationToken.None);
